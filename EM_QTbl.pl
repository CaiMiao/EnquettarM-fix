...
###### Modified by CaiMiao ################
#
# - fixed the last empty entry bug(?)
# - now recognizes input file name
# - this altered script generates csv compatible with QTblEditor

use strict;

$/ = v0;
my $input;
my $outputPrefix = (split(/\./,@ARGV[1]))[0];  # extract input file name without suffix
my $outputtext = $outputPrefix.".csv";         # generate .csv suffix
my $outputtable = $outputPrefix.".tbl";        # generate .tbl suffix
...
if ($eamode eq "-e")  ### Export data to txt file
{
    ...
    foreach my $i (0 .. $numElements - 1)   # Same thing as last time, $i goes from 0 to 1 less than the number of elements.
    {
    	...
    	$values[$i] = $string;    # Set entry number $i in array @values to equal $string
          ...
          print FOUT "\"${inkey}\",\"${inval}\"\n";  # Add line to the txt file, sperated by comma
   }
    print "Done!\n";
...
elsif ($eamode eq "-i")  #### Import data from txt file
{
    my @keys;                  # Creates an array named @keys.
    my @values;                # Creates an array named @values.
    my $numElements = 0;       # ( fix: $numElements++ caused 1 empty entry )
	...
	READDATA: while (<DATA>)   # Read in a line at a time from the data file, until there are no more lines, running this loop on each line
	{
		...
		$value =~ s/\\n/\n/g;    # Replace every instance of the character "\n" in the value with an actual new line.
          ...
          $key =~ s/\\n/\n/g;      # Replace every instance of the character "\n" in the key with an actual new line.
